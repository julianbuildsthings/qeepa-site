...
- **Section reveals** are CSS, and apply only under `.js` (added before first paint) and `prefers-reduced-motion: no-preference`. They fade and rise 16px over `--motion-editorial` with `--ease-enter`, visual first and copy one `--reveal-stagger` behind. They reveal once via IntersectionObserver, and everything shows at once without it.
