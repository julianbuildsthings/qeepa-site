...
- **Back** (legal pages only): the app's Back button as its bar draws it — 36px round, surface-2 fill, 15px chevron, 6px in from the bar's left edge (the bar's left padding tightens from 20px to match) — a link home rather than `history.back()`, so it lands on the home page however the visitor arrived. The home page has none.
